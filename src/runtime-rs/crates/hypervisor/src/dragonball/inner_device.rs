...
use crate::{
    device::DeviceConfig, HybridVsockConfig, NetworkConfig, ShareFsDeviceConfig,
...
impl DragonballInner {
    pub(crate) async fn add_device(&mut self, device: DeviceConfig) -> Result<()> {
        ...
        match device {
            ...
            DeviceConfig::HybridVsock(config) => self.add_vsock(&config).context("add vsock"),
    ...
    fn add_vsock(&mut self, config: &HybridVsockConfig) -> Result<()> {
